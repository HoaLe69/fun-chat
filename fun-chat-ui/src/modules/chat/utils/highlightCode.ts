...
export async function getCodeParser(languageName: string, defaultLanguage?: Language): Promise<Parser | null> {
...
export default async function highlightCode<Output>(
  languageName: string,
  input: string,
  ...
  if (parser) {
    ...
    pos != tree?.length && output.push(callback(input.slice(pos, tree?.length), null, pos, tree?.length))
    return output
  } else {
